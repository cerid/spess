...
var/controller/sorter/Sort = new()
/controller/sorter
	...
	Init(_L, _cmp=/proc/cmp_numeric_asc, _associative)
		L = _L
		cmp = _cmp
	...
	proc/Tim(start, end)
		...
		if(remaining < MIN_MERGE)
			...
			Binary(start, end, start+initRunLen)
			return
		...
		do
			...
			if(runLen < minRun)
				...
				Binary(start, start+force, start+runLen)
				runLen = force
	...
	*/
	proc/Binary(lo, hi, start)
		...
		for(,start < hi, ++start)
			...
			var/insertWhere = FindInsertPos(lo, start, pivot)

			moveElement(L, start, insertWhere)	//move pivot element to correct location in the sorted range
	...
	*/
	...
	proc/Merge(start, end)
		...
		do
			...
			Binary(start, start+runLen, start)
